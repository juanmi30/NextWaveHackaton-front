...
const watchActivity = (watch: PreventiveWatch): AgentActivityItem[] => [{ id: `${watch.id}-signal`, label: 'WATCH', title: 'Predictive signal detected', detail: 'An estimated degradation risk was returned for this payment scope.', tone: 'warning' }, ...(watch.riskScore === undefined ? [] : [{ id: `${watch.id}-score`, label: 'RISK', title: 'Estimated degradation risk available', detail: `Estimated degradation risk: ${watch.riskScore.toFixed(2)}.`, tone: 'warning' as const }]), { id: `${watch.id}-investigating`, label: 'WATCH', title: 'Preventive investigation active', detail: 'The Watchtower is gathering evidence and awaiting Detection confirmation.', tone: 'running' }]

...
export function AgentEventItem({ event, highlighted = false }: { event: AgentEvent; highlighted?: boolean }) {
  ...
  return <article className={`agent-event agent-${event.status} ${highlighted ? 'highlighted' : ''}`}>
    <span className="agent-event-dot" aria-hidden="true" /><div className="agent-event-card">
      ...
      {event.decision ? <div className="agent-event-details"><span>Risk <strong>{event.decision.riskLevel ?? '--'}</strong></span><span>Confidence <strong>{event.decision.confidence !== undefined ? `${event.decision.confidence}%` : '--'}</strong></span><span>Action <strong>{event.decision.action ?? '--'}</strong></span></div> : null}
    </div>
  </article>
}

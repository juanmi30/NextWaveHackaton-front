...
export function AgentSummaryCard() {
  const { events, latestEvent, isRunning, currentPhase } = useAgentStreamContext()
  ...
  const activity = !latestEvent ? 'Waiting for activity...' : isRunning && currentPhase ? activityLabel[currentPhase] : 'Analysis completed'
  ...
  return <article className="panel agent-summary">
    ...
    <a className="button secondary agent-summary-link" href="#/agent-live">View live activity →</a>
  </article>
}

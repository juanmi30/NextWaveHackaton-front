...
export function DemoControls() {
  ...
  const statusText = demoStatus === 'RUNNING' ? (isSse ? 'AI analysis running' : 'Running demo...') : demoStatus === 'PAUSED' ? 'Demo paused' : demoStatus === 'COMPLETED' ? (isSse ? 'Diagnosis ready' : 'Analysis completed') : demoStatus === 'ERROR' ? 'Analysis failed' : isSse && incidentId ? 'AI analysis queued' : 'Ready to start'
  return <article className="panel demo-controls">
    <div className="demo-control-row"><div><span className="metric-label">{isSse ? `Incident analysis${incidentId ? ` · ${incidentId}` : ''}` : 'Demo scenario'}</span><strong>{statusText}</strong><span className="metric-detail">{isSse ? `${events.length} observable events received` : `Scenario progress · ${events.length} / 7`}</span>{error ? <span className="agent-control-error">{error}</span> : null}</div><div className="actions">
      ...
      {demoStatus === 'ERROR' ? <button type="button" className="button secondary" onClick={startDemo}>{isSse ? 'Retry analysis' : 'Start demo'}</button> : null}
      ...
      {demoStatus === 'COMPLETED' && isSse ? <button type="button" className="button primary" onClick={() => document.querySelector('.diagnosis-panel')?.scrollIntoView({ behavior: 'smooth', block: 'start' })}>View diagnosis</button> : null}

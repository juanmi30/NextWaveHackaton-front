...
export function RoutingRecommendation({ recommendation, source }: { recommendation: Recommendation; source: RouteHealth | null }) {
  ...
  return <article className="panel routing-recommendation">
    ...
    <div className="recommendation-metrics"><div><span>Approval</span><strong>{candidate.approvalRate}%</strong></div><div><span>Current route</span><strong>{source ? `${source.currentApproval}%` : '--'}</strong></div><div><span>Expected recovery</span><strong>{recommendation.expectedApprovalRecovery !== undefined ? `+${recommendation.expectedApprovalRecovery} pp` : '--'}</strong></div><div><span>Confidence</span><strong>{recommendation.confidence !== undefined ? `${recommendation.confidence}%` : '--'}</strong></div><div><span>Score</span><strong>{candidate.score} / 100</strong></div></div>

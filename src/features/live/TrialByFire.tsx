...
export function TrialByFire({ degradations, refreshedAt, onChanged }: { degradations: LiveDegradation[]; refreshedAt: number; onChanged: () => Promise<void> }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try { await addLiveDegradation({ dimensions, approvalRate: Number(form.approvalRate), durationSeconds: Number(form.durationSeconds), failureReason: form.failureReason.trim() || undefined }); setMessage('Degradation active. Waiting for automatic detection.'); await onChanged() }

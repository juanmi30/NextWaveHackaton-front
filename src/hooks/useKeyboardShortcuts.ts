...
type ShortcutOptions = { navigate: (page: 'overview' | 'incidents' | 'transactions' | 'agent-live' | 'ml-metrics') => void; refresh?: () => void }
...
export function useKeyboardShortcuts({ navigate, refresh }: ShortcutOptions) {
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      const pages = { '1': 'overview', '2': 'incidents', '3': 'transactions', '4': 'agent-live', '5': 'ml-metrics' } as const
